...
expt_list=c("CO15-MBO-NC-20210816",
...
for (expt in 1:length(expt_list)){
  #manual
  name <- expt_list[expt]
  path <- paste0("/data/GI-Informatics/DePasquale/Projects/Shivakumar_MBO/QC/", name, "/", name, "_filtered_matrices")
  ...
  pdf("TSNE-plot.pdf", width = 10, height = 8)
  M <- RunTSNE(object = M, dims = 1:20)
  ...
  pdf(file = paste0(name, "_TSNE_Seurat.pdf"), width = 6, height = 6)
  par(mar=c(4, 4, 4, 4))
